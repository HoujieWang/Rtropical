ALP2 = function(data,CV = F){
  ...
  if (CV == F){
    ...
    accu = lapply(1:nrow(indice_box),function(x){A2(data,tst_data,indice_box[x,])$best_accuracy})
    indice_num = which.max(accu)
    op_omega = A2(data,tst_data,indice_box[indice_num,])$omega
    op_accu = A2(data,tst_data,indice_box[indice_num,])$best_accuracy
    ...
    for (i in 1:k){
      ...
      accu = lapply(1:nrow(indice_box),function(x){A2(trn_data,tst_data,indice_box[x,])$best_accuracy})
      indice_num = which.max(accu)
      omega = A2(trn_data,tst_data,indice_box[indice_num,])$omega
      accu = A2(trn_data,tst_data,indice_box[indice_num,])$best_accuracy
      if (accu > op_accu){
        op_accu = accu
        op_omega = omega
      }
